...
/////////////////////[[ FUNÇÕES OUVINTES, QUE VEM DOS CLIENTES ]]/////////////////////
				...
				socket.on('update_movimento', update_movimento);
...
************************************************************************************/
 ...
 function logar(data){
	 ...
	 var consulta = 'SELECT count(id) AS validarAcesso, id, nick, size_w, size_h, pos_up, pos_left, anima_p FROM cadastro WHERE login = ? and senha = ?'; // SQL
		 ...
		 if (err){
			...
			console.log('Aconteceu o seguinte erro: '+ err);
			 ...
			 if(rows[0].validarAcesso>0){
				...
				var cliente = {nome: rows[0].nick, idSocket: servidor.id, id: rows[0].id, size_w: rows[0].size_w, size_h: rows[0].size_h, pos_up: rows[0].pos_up, pos_left: rows[0].pos_left, anima_p: rows[0].anima_p, lado_p: 0, cmd_p_temp: 0, m_right: 1, m_left: 2, m_up: 3, m_down: 4, m_enter: 5}
...
function sucesso_ao_logar(){
	...
	var cliente_logado = clientesOnline[nClienteDesconectado];
	servidor.emit('sucesso_ao_logar_c', {cliente_logado : cliente_logado});
}
...
function update_movimento(data){
	var coord_x = data.mov_x;
	var coord_y = data.mov_y;
	
	var clienteSel = servidor.id;
	var jogador_mover = identificarCliente(clienteSel);
	
	coord_x = clientesOnline[jogador_mover].pos_left = coord_x;
	coord_y = clientesOnline[jogador_mover].pos_up = coord_y;
	
	socket.broadcast.emit('r_movido_player',{clientesOnline: clientesOnline});
	socket.emit('r_movido_player',{clientesOnline: clientesOnline});
		...
		conexaoMysqlGameYugioh.query(consulta, function(err, rows, fields) {
			...
			if (err){
				...
				if(rows.length>0){
						...
						cartaRetornar = {nome: rows[0].nome, atak: rows[0].atk, def: rows[0].def, imagem: rows[0].imagem, idCarta: cartaRecebida}
						socket.broadcast.emit('retonrarConsultarCarta',{cartaRetonada: cartaRetornar, hostGuest: hostGuest});
				}else{
			...
			console.log('Aconteceu o seguinte erro: '+ err);
			 ...
			 if(rows[0].validarAcesso>0){
				...
				if(verificarSeClienteEstaOnline(rows[0].id)){
					...
					var cliente = {nome: rows[0].nick, idSocket: servidor.id, id: rows[0].id, cor:corCliente, avatar:rows[0].avatar, idFacebook: rows[0].idFacebook}
...
************************************************************************************/	
	function enviarMensagemDoChat(data){
		...
		var msgRegistrar = {nCliente:nomeDoCliente, cmsg: msg, idCliente:idDoCliente, corDoCliente: corDoCliente, avatarDoCliente:avatarDoCliente, idfacebook: idfacebook};
...
************************************************************************************/		
	...
	function consultar_resposta_pet(nClienteDesconectado, id_cliente, msg_cliente, prox_increment_frases){
		...
		var msgRegistrar_ret = {nCliente:clientesOnline[nClienteDesconectado].petN+" ", cmsg: msg_resp, idCliente:clientesOnline[nClienteDesconectado].id, corDoCliente: "#ffffff", avatarDoCliente:"avatar", idfacebook: "123"};
		 ...
		 if (err){
			...
			console.log('Aconteceu o seguinte erro: '+ err);
			 ...
			 if(rows[0].validarAcesso>0){
					...
					servidor.emit('retornarSalasDeDuelos', {SalasDeDueloss: SalasDeDuelos, idFacebook: rows[0].idFacebook});
		 ...
		 if (err){
			...
			console.log('Aconteceu o seguinte erro: '+ err);
			 ...
			 if(rows[0].validarAcesso>0){
					...
					var salaDeDuelo = {id: numeroDaSala, nomeDSala: nomeDaSala, clienteHost: clienteHost, avatarHost: avatarDoCliente, clienteGuest: 0, pontosIni:8000, modalidade:1, status:0};
					...
					servidor.emit('salaDeDueloCriada',{avatarDoCliente: avatarDoCliente, nome: rows[0].nick, numerDaSala: numeroDaSala, nomeDaSala: nomeDaSala});
			...
			if (err){
				...
				if(rows.length>0){
					...
					if(numeroDaPartida != undefined){
						...
						servidor.emit('retornarInicioDoDuelo', {clienteHost: clienteHost, clienteGuest: clienteGuest, Npartida: numeroDaPartida});
						servidor.broadcast.emit('retornarInicioDoDuelo', {clienteHost: clienteHost, clienteGuest: clienteGuest, Npartida: numeroDaPartida});
	...
	function inciarGame(idDaSala){
		...
		var partida = {id: numeroDaPartida, lps: pontosIniciais, modalidade: modalidade, status:3, deckHost:0, deckGuest:0, clienteGuest: clienteGuest, clienteHost: clienteHost}
...
************************************************************************************/	
	function desconectarCliente(data){
		...
		if(nClienteDesconectado != undefined){			
			...
			if(nDaSala != undefined){
				...
				if(statusDaSala == 0){
					...
					if((idDoCliente == ClienteHostDaSala) || (idDoCliente == ClienteGuestDaSala)){
						...
						servidor.broadcast.emit('rediceionarPorDeconexaoDeSala',{pagina: 'http://portalyugioh.com.br/conta/home/salas', oponente: sujeitoRedirecionado})
					}
				...
				if(nDaPartida != undefined){
					...
					if(statusDaPartida == 7){
						...
						servidor.broadcast.emit('rediceionarPorDeconexaoDeSala',{pagina: 'http://portalyugioh.com.br/jogo/1.0/conta.php?pagina=salas', oponente: sujeitoRedirecionado});
			...
			if (nClienteDesconectado > -1) { clientesOnline.splice(nClienteDesconectado,1); }
