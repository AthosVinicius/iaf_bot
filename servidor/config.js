...
function fct_enviar_msg_chat(fala_passada) {
    ...
    if (!disponi) {
        ...
        if (remover_espacos_str(cmp_msg).toUpperCase() == pet_nome.toUpperCase()) {
            alterar_disponi();
            msg_personalizada(pet_nome, "o'que deseja " + nick_armazenado + "?", verificar_lado_msg(pet_nome));
...
function remover_espacos_str(string) {
...
function confirmacaoDaMensagemEnviadaEu_pet(data) {
    ...
    $("#char_assistente").css('background-image', 'url(http://4.bp.blogspot.com/_DxtfvSvSCK8/TOPuWb45c1I/AAAAAAAACEU/mFZeDW2Hn_s/s320/digitando.gif)')
        .delay(tmp_resp).queue(function (next) {
        atualizar_log_msg(data.msgRt, tmp_resp);

        $("#char_assistente").css("background-image", "url('http://th02.deviantart.net/fs70/PRE/f/2012/054/8/7/adventure_time_wallpaper___beemo_face___1920x1080_by_finlay_mckinnon-d4qpz0a.jpg')");
...
function env_com_enter(e) {
    ...
    if (keycode == 13) {
        fct_enviar_msg_chat($("#input_cmp_msg").val());
    }
...
function alterar_disponi() {
    ...
    tempo_contador_descanso = setInterval(function () {
        contador_de_descanso(ver_diferenca_d_minutos(tempo_ultma_msg, pegar_hora_minuto_atual()))
    }, 10000);
...
function alterar_disponi2() {
    ...
    $("#char_assistente").html("<div id='dormindo_ch'><img style='  margin-top: -23px; padding-left: 77px;' src='http://1.bp.blogspot.com/-2cix9W9uDLg/UZJzc2K5lJI/AAAAAAAADrQ/1kNNdztz890/s1600/zzz-facebook-symbol.png'></div>");
}
